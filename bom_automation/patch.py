...
def render_comparison_tab():
    ...
    # ── DIAGNOSTIC: Costing Detail & Supplier Lookup Inspector ──────────────
    render_divider()
    with st.expander("🔬 Diagnose Supplier Lookup (open before running validation)", expanded=False):
        diag_style = st.selectbox("Inspect BOM", options=list(bom_dict.keys()), key="diag_style_sel")
        diag_bom = bom_dict.get(diag_style, {})
        diag_costing = diag_bom.get("costing_detail", pd.DataFrame())
        diag_lookup  = diag_bom.get("supplier_lookup", {})

        dc1, dc2 = st.columns(2)
        with dc1:
            st.markdown(f"**Supplier Lookup dict — {len(diag_lookup)} entries**")
            if diag_lookup:
                st.dataframe(
                    pd.DataFrame([{"Code": k, "Supplier": v} for k, v in sorted(diag_lookup.items())]),
                    use_container_width=True, height=250
                )
            else:
                st.error("Supplier lookup is EMPTY — costing detail was not parsed.")

        with dc2:
            if not diag_costing.empty:
                st.markdown(f"**Costing Detail — {len(diag_costing)} rows**")
                st.markdown(f"Columns: `{list(diag_costing.columns)}`")
                st.dataframe(diag_costing.head(20), use_container_width=True, height=250)
            else:
                st.error("Costing detail DataFrame is EMPTY.")

        st.markdown("**Enter a material code to test lookup:**")
        test_code = st.text_input("Material code to test (e.g. 067535)", key="diag_code_input")
        if test_code:
            from validators.filler import _find_supplier
            from parsers.pdf_parser import _build_supplier_lookup
            result_cached = diag_lookup.get(test_code.strip(), "NOT IN DICT")
            result_scan   = _find_supplier(diag_costing, test_code.strip())
            st.markdown(f"- **Dict lookup** for `{test_code}`: `{result_cached}`")
            st.markdown(f"- **Full scan** for `{test_code}`: `{result_scan}`")
            # Show rows where the code appears
            if not diag_costing.empty:
                mask = diag_costing.apply(
                    lambda r: r.astype(str).str.contains(re.escape(test_code.strip()), na=False).any(), axis=1
                )
                hits = diag_costing[mask]
                st.markdown(f"- **Rows containing `{test_code}`**: {len(hits)}")
                if not hits.empty:
                    st.dataframe(hits, use_container_width=True)
                else:
                    st.warning("Code not found in ANY cell of costing detail — check the PDF section detection.")
